...
from data.waterbird import get_waterbird_datasets
...
get_dataset_funcs = {
    ...
    'waterbird': get_waterbird_datasets,
    'scars': get_scars_datasets,
    'aircraft': get_aircraft_datasets,
    'pku-aircraft': get_pku_aircraft_datasets, 
    'imagenet21k': get_image_net_datasets, 
}

def get_datasets(name, transform='default', image_size=224, train_classes=(0, 1, 8, 9), open_set_classes=range(10), balance_open_set_eval=False, split_train_val=True, seed=0, osr_split=None, args=None):
    ...
    if name in get_dataset_funcs.keys():
        if name == 'imagenet21k':
            datasets = get_dataset_funcs[name](train_transform, test_transform, train_classes=train_classes, open_set_classes=open_set_classes, balance_open_set_eval=balance_open_set_eval, split_train_val=split_train_val, seed=seed, osr_split=osr_split)
        else:
            datasets = get_dataset_funcs[name](train_transform, test_transform, train_classes=train_classes, open_set_classes=open_set_classes, balance_open_set_eval=balance_open_set_eval, split_train_val=split_train_val, seed=seed)
    else:
...
def get_class_splits(dataset, split_idx=0, cifar_plus_n=10):
    if dataset in ('cifar-10-10', 'mnist', 'svhn'):
        ...
        osr_path = os.path.join(osr_split_dir, 'imagenet21k_osr_splits.pkl')
        with open(osr_path, 'rb') as f:
            class_info = pickle.load(f)

        train_classes = class_info['known_classes']
        open_set_classes = class_info['unknown_classes']
        open_set_classes = open_set_classes['Hard'] + open_set_classes['Easy']
