...
from config import cub_root, waterbird_root
...
class CustomCub2011(Dataset):
    ...
    def __init__(self, root, wb_root=None, train=True, transform=None, target_transform=None, loader=default_loader, download=True):

        self.root = os.path.expanduser(root)
        self.wb_root = wb_root
    ...
    def _check_integrity(self):
        ...
        for index, row in self.data.iterrows():
            filepath = os.path.join(self.wb_root, row.filepath)
    ...
    def __getitem__(self, idx):
        ...
        path = os.path.join(self.wb_root, sample.filepath)
...
def get_waterbird_datasets(train_transform, test_transform, train_classes=range(160),
    ...
    train_dataset_whole = CustomCub2011(root=cub_root, wb_root=waterbird_root, transform=train_transform, train=True)
    ...
    test_dataset_known = CustomCub2011(root=cub_root, wb_root=waterbird_root, transform=test_transform, train=False)
    test_dataset_known = subsample_classes(test_dataset_known, include_classes=train_classes)

    # Get testset for unknown classes
    test_dataset_unknown = CustomCub2011(root=cub_root, wb_root=waterbird_root, transform=test_transform, train=False)
...
if __name__ == '__main__':
    ...
    x = get_waterbird_datasets(None, None, split_train_val=False, train_classes=np.random.choice(range(200), size=100, replace=False))
