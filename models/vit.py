...
class VisionTransformer(nn.Module):
    ...
    def get_nth_selfattention(self, x, nth_att=None, cls_token=None):
        x = self.prepare_tokens(x)

        if nth_att is None:
            nth_att = len(self.blocks) - 1
    ...
    def extract_features_and_attn(self, x, nth_att=None):
        features_list = self.get_intermediate_layers(x, cls_token)
        features = torch.cat([x[:, 0] for x in features_list], dim=-1)
        attn = self.get_nth_selfattention(x, nth_att)
...
class LinearClassifier(nn.Module):
    ...
    def forward(self, x):
        features_list = self.base_model.get_intermediate_layers(x, 4)
        feat = torch.cat([x[:, 0] for x in features_list], dim=-1)
