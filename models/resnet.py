...
import numpy as np
from utils.stools import ash_b
...
class ResNetCifar(AbstractResNet):
    def __init__(self, block, layers, num_classes=10, feat_dim=512):
    ...
    def features(self, x, which_layer=4):
        out = F.relu(self.bn1(self.conv1(x)))
        l1 = self.layer1(out)
        if which_layer == 1:
            return l1
    ...
    def forward(self, x, return_feat=False, using_ash=False):
    ...
    def forward_threshold(self, x, threshold=1e10, return_feat=False, using_ash=False):
