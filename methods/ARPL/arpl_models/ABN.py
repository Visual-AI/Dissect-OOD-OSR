...
class _MultiLayerNorm(nn.Module):
    _version = 2

    def __init__(self, num_features, num_classes, eps=1e-5, affine=True):
        super(_MultiLayerNorm, self).__init__()
        self.lns = nn.ModuleList(
            [nn.LayerNorm(num_features, eps, affine) for _ in range(num_classes)])
...
class MultiLayerNorm(_MultiLayerNorm):
    def _check_input_dim(self, input):
        if input.dim() != 4:
            raise ValueError('expected 4D input (got {}D input)'.format(input.dim()))