...
def resnet18ABN(num_classes=10, num_bns=2, feat_dim=512):
    model = ResNetABN(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, num_bns=num_bns, feat_dim=feat_dim)
...
def resnet50ABN(num_classes=10, num_bns=2, first_layer_conv=3, feat_dim=2048):
    model = ResNetABN(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, num_bns=num_bns, first_layer_conv=first_layer_conv)
...
class ResNetABN(nn.Module):
    def __init__(self, block, layers, num_classes=10, num_bns=2, first_layer_conv=3, feat_dim=512):
        self.inplanes = 64
    ...
    def features(self, x, domain_label=None, which_layer=1):
        if domain_label is None:
            domain_label = 0 * torch.ones(x.shape[0], dtype=torch.long).to(x.device)
        x = self.conv1(x)
        x, _ = self.bn1(x, domain_label)
        x = F.relu(x)
        l1, _ = self.layer1(x, domain_label)
        if which_layer == 1:
            return l1
        l2, _ = self.layer2(l1, domain_label)
        if which_layer == 2:
            return l2
        l3, _ = self.layer3(l2, domain_label)
        if which_layer == 3:
            return l3
        l4, _ = self.layer4(l3, domain_label)
        if which_layer == 4:
            return l4

    def feature_list(self, x, domain_label=None):
        if domain_label is None:
            domain_label = 0 * torch.ones(x.shape[0], dtype=torch.long).to(x.device)
        x = self.conv1(x)
        x, _ = self.bn1(x, domain_label)
        l0 = F.relu(x)
        # l0_list.append(l0)
        l1, _ = self.layer1(l0, domain_label)
        # out_list.append(out)
        l2, _ = self.layer2(l1, domain_label)
        # out_list.append(out)
        l3, _ = self.layer3(l2, domain_label)
    ...
    def forward_threshold(self, x, threshold=1e10, domain_label=None, return_feat=False):
        if domain_label is None:
            domain_label = 0 * torch.ones(x.shape[0], dtype=torch.long).to(x.device)
        x = self.conv1(x)
        x, _ = self.bn1(x, domain_label)
